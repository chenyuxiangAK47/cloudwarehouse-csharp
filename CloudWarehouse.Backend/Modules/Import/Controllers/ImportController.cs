...
public class ImportController : ControllerBase
{
    ...
    /// <summary>导入：自动补齐站点/目的地主数据，按线路整批替换价格规则。</summary>
        ...
        IFormFile? file, bool saveToDatabase)
    {
        ...
        {
            var hint = ex.Message.Contains("IX_PriceRules_SiteId_DestId_EffectiveDate", StringComparison.OrdinalIgnoreCase)
                ? " 请执行 database/fix-price-rules-index.sql。"
                : " 请确认已建库且连接字符串正确。";
            return Ok(ApiResponse<PriceTableImportResult>.Fail($"数据库错误: {ex.Message}。{hint}"));
        }
        catch (Exception ex)
        {
